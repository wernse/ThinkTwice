...
public class MainActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(i.getSerializableExtra("list") != null){
        	int totalCalories = instantiateList();
        	...
        	String totalCaloriesString = "Total Calories: "+totalCalories+" out of 2000cals";
        	//Log.v(TAG, "totalCaloriesString"+ totalCaloriesString);
        	textView.setText(totalCaloriesString);//view->
        	Log.v(TAG, "PRECOMPARE");
        	if(i.getSerializableExtra("compare") != null){
        		Log.v(TAG, "COMPARE");
            	...
            	ListAdapter compareListAdapter = new SimpleAdapter(this, compareItems, R.layout.item,
            			new String[] { KEY_NAME, KEY_DESC, KEY_COST }, new int[] {
    				 R.id.name, R.id.description, R.id.type });
            	compareListView.setAdapter(compareListAdapter); 
            	double metricValue=0.0;
            	if(metric[0].equals("1/2")){
            		metricValue = 0.5;
            	}else{
            		metricValue = Integer.parseInt(metric[0]);
            	}
        	...
        	double calorieBar = totalCalories/2000.0;
        	final int caloriePercent = (int) (calorieBar*100);
            // Start lengthy operation in a background thread
            new Thread(new Runnable() {
                public void run() {
                    while (mProgressStatus < caloriePercent) {
                    	mProgressStatus += 1;
